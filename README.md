...
[1. データ収集] pnpm collect
  |  gen/data_source/yyyy_mm_dd_hh_mm_ss/news.json
  v
[2. キーワード抽出] pnpm extract
  |  gen/data_source/yyyy_mm_dd_hh_mm_ss/keyword.json
  v
[3. 要件生成] pnpm generate
  |  gen/requirements/{app_name}/overview.md, features/*.md
  v
...
- `YOUTUBE_API_KEY` - [YouTube Data API](https://developers.google.com/youtube/v3) のAPIキー（`app.config.yaml` で `enabled: false` がデフォルト）
...
| コマンド | 説明 |
| --- | --- |
| `pnpm collect` | 外部APIからトレンドデータを収集し `gen/data_source/` に保存 |
...
│   ├── agents/               # カスタムエージェント定義
│   └── skills/               # カスタムスキル定義
├── app.config.yaml           # アプリケーション設定（フェーズ別）
...
`app.config.yaml` でフェーズごとの設定を階層管理します。`collect.sources` 配下にデータソースごとの有効/無効、APIエンドポイント、パラメータを定義。
