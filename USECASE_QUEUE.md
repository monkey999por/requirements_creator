...
ユーザーのジャストアイデアを保存しておき、定期実行（Viewer内蔵スケジューラ）で自動的に要件生成するキュー機能のドキュメント。
...
- 定期実行時（Viewer内蔵スケジューラ）にキューが自動で処理され、要件が生成される
...
Viewer内蔵スケジューラ（croner）が `scheduler-run.sh` を定期実行する。キューの有無で処理を分岐:
...
スケジューラの有効化はViewerの「スケジューラ設定」画面から操作する。
