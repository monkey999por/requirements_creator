...
```json
{
  ...
  "perspectives": {
    "mode": "{single | combine | random}",
    "items": ["{適用された観点1}", "{適用された観点2}"]
  },
...
**生成観点**: `perspectives` フィールドはオプション。生成観点が指定されている場合のみ記録する。
- `mode`: `single`（1つの観点を適用）/ `combine`（複数の観点を掛け合わせ）/ `random`（ランダム選択）
- `items`: 実際に適用された観点の配列。有効な値:
  - `kindness` — ユーザーに寄り添う優しいUX設計
  - `cunning` — 巧妙なマネタイズ・行動設計
  - `frustration` — フラストレーション駆動の設計
  - `dopamine` — 中毒性のある体験設計
  - `target-focus` — ターゲットユーザー層に特化した設計
