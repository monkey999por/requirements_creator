...
overview.mdと各機能の仕様に基づき、Mermaid記法を使ったアプリケーション設計図を `diagrams.md` として出力する。

[テンプレート参照](templates.md) の「diagrams.md」セクションに従い、以下の図解を **全て** 含むこと:

1. **画面遷移図**: 主要画面間のナビゲーション遷移をMermaid `graph` で記述
2. **ユーザーフロー図**: メインのユースケース（2〜3シナリオ）を `sequenceDiagram` で記述
3. **システム構成図**: フロントエンド・バックエンド・DB・外部サービスの構成をMermaid `graph` で記述
...
このスクリプトは以下を自動検証する:
- `_source_info.json`, `overview.md`, `diagrams.md`（Mermaid記法）, `features/` ディレクトリの存在
