...
**生成観点**: `app.config.yaml` の `generate.perspectives` に設定がある場合、またはプロンプト内で生成観点が指定されている場合は、その観点をアプリの体験設計・機能設計・マネタイズ戦略に深く反映する。定義済み観点: `kindness`（優しいUX）, `cunning`（巧妙なビジネス設計）, `frustration`（フラストレーション駆動）, `dopamine`（中毒性設計）, `target-focus`（ニッチ特化）。適用した観点は `_source_info.json` の `perspectives` フィールドに記録する。
...
- `perspectives`（オプション）: 生成観点が指定されている場合、適用した観点を記録する（`mode`, `items`）
...
### ステップ6a: features/_meta.json の書き出し

ステップ6で書き出した全featureファイルのメタデータを `features/_meta.json` に出力する。

[テンプレート参照](templates.md) の「features/_meta.json」セクションに従い、各featureの `id`, `filename`, `title`, `summary` を配列で記録する。

- `title`: overview.md の機能一覧テーブルに記載した機能名
- `summary`: 各featureファイルの「## 概要」セクションの最初の1行

**出力先**: `gen/requirements/{app_name}/features/_meta.json`
...
### ステップ6.5a: diagrams/_meta.json の書き出し

ステップ6.5で書き出した全diagramファイルのメタデータを `diagrams/_meta.json` に出力する。

[テンプレート参照](templates.md) の「diagrams/_meta.json」セクションに従い、各diagramの `id`, `filename`, `title` を配列で記録する。

- `title`: 各diagramファイルの `# タイトル` 見出しの内容

**出力先**: `gen/requirements/{app_name}/diagrams/_meta.json`
